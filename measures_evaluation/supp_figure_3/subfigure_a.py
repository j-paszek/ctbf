...
def create_subfigure_a(
    ...
    distances = load_distance_vectors(results_dir, run_id=run_id, include_naive=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Appendix Figure 3 subfigure A")
    parser.add_argument("--results-dir", type=str, default=str(default_results_dir()))
    parser.add_argument("--output-path", type=str, default=str(default_figures_dir() / "subfigure_a.png"))
    parser.add_argument(
        "--run-id",
        type=int,
        default=None,
    ...
    output_path = create_subfigure_a(args.results_dir, args.output_path, run_id=args.run_id)
    print(f"Saved subfigure A to {output_path}")
